...
function App() {
    ...
    axios.get('https://api.pokemontcg.io/v2/cards', {
      ...
      .then(response => {
        if (response.data && response.data.data) {
          ...
          Math.ceil(response.data.total / cardsPerPage)
        } else {
  ...
  return (
    <div className="relative min-h-screen mb-20 ml-20 mr-20 sm:mx-8 md:mx-16 lg:mx-20">
    <nav className="bg-gray-800 p-6 text-white h-24 shadow-2xl shadow-slate-400 flex items-center fixed top-0 left-0 right-0 z-50">
      <div className="flex items-center mb-4 sm:mb-0">
        <img
          src={poke}
          alt="Pokémon Icon"
          className="h-16 w-16 mr-4 mb-4 ml-4 mt-6 sm:h-16 sm:w-16 sm:mr-4"
        />
        <h1 className="text-3xl font-bold mb-6 mt-4">Pokédex</h1>
      </div>
      <div className="flex-grow flex justify-center sm:p-2 ml-16 mr-12">
        <input
          type="text"
    ...
    <div className="container mx-auto p-4 mt-28 mb-12">
      <div className="flex justify-end mb-4">
        <select
          ...
          value={sortBy}
          className="sm:mr-8 p-2 border-2 rounded-3xl bg-gray-200 transition-transform duration-300 ease-in-out transform hover:scale-105 inner-shadow-3xl shadow-2xl shadow-gray-300"
        >
        ...
        {loading ? (
          ...
          currentCards.map(card => (
            ...
            >
              ...
              <div className="absolute bottom-2 right-2 text-gray-500 rounded-2xl w-12 mb-2 h-8 flex items-center justify-center sm:right-1 sm:w-14 sm:h-6 md:w-16 md:h-8 lg:w-18 lg:h-8 xl:w-20 xl:h-8">
        ...
        <div className="flex space-x-2">
          {Array.from({ length: totalPages }, (_, index) => (
            <button
              ...
              className={`px-4 py-2 border-2 hover:border-t-2 shadow-2xl rounded-full transition-transform duration-300 ease-in-out transform hover:scale-125 ${currentPage === index + 1 ? 'bg-gray-500 text-stone-100 hover:bg-gray-600 hover:text-stone-100 active:bg-gray-700' : 'bg-stone-100 hover:bg-gray-600 hover:text-stone-100 text-gray-700'}`}
            >
              {index + 1}
            </button>
          ))}
